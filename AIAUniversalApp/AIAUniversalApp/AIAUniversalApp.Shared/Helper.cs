...
namespace AIAUniversalApp.Shared.Helpers
{
    public class Helper
    {
        ...
        public static List<T> Get<T>()
        {
            using (var client = new HttpClient())
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    return Deserialize<List<T>>(response.Content.ReadAsStringAsync().Result);
                }
                else throw new System.Exception(response.Content.ReadAsStringAsync().Result);
        ...
        public static List<Quote> GetSearch(string type, string query)
        {
            using (var client = new HttpClient())
            {
                string url = String.Format("{0}/api/search/{1}/{2}", apiDomain, type, query);
                HttpResponseMessage response = client.GetAsync(url).Result;

                if (response.IsSuccessStatusCode)
                {
                    return Deserialize<List<Quote>>(response.Content.ReadAsStringAsync().Result);
                }
                else throw new System.Exception(response.Content.ReadAsStringAsync().Result);
            }
        }

        public static List<Quote> GetMultipleSearch(Dictionary<string,string> queries)
        {
            using (var client = new HttpClient())
            {
                ...
                {
                    queryString += String.Format("{0}={1}&", WebUtility.UrlEncode(q.Key), WebUtility.UrlEncode(q.Value));
                }
                string url = String.Format("{0}/api/search/multiple/?{1}", apiDomain, queryString);
                HttpResponseMessage response = client.GetAsync(url).Result;
